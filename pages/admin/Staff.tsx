...
export const Staff = () => {
  const { users, addUser, updateUser, deleteUser, user: currentUser, currentStore } = useApp();
  ...
  const handleAddPayrollRow = () => {
    ...
    const newUser: User = {
      ...
      hr: { baseSalary: 0, workDays: 0, totalWorkHours: 0, leaveDays: 0, bonus: 0, penalty: 0, cashAdvance: 0 }
    };
    addUser(newUser);
  };
  ...
  const calculateTotalSalary = (hr?: HRDetails) => {
    if (!hr) return 0;
    // Công thức: Lương cứng + Thưởng - Phạt - Tạm ứng
    const total = hr.baseSalary + hr.bonus - hr.penalty - (hr.cashAdvance || 0);
    // Ở bảng lương cho phép hiển thị số âm (nợ quán) để dễ theo dõi
    return total;
  };
    ...
    <div className="p-6 h-full overflow-y-auto">
      <div className="flex justify-between items-center mb-6">
        <div>
            ...
            <p className="text-gray-400 text-sm mt-1">Quản lý lương thưởng, phạt và tạm ứng cho nhân viên tại cơ sở này.</p>
        </div>
      </div>
            ...
            <table className="w-full text-left border-collapse text-sm">
              <thead className="bg-gray-700 text-gray-300">
                <tr>
                  ...
                  <th className="p-3 border-r border-gray-600 min-w-[150px]">Họ tên Nhân viên</th>
                  <th className="p-3 border-r border-gray-600 text-right min-w-[120px]">Lương Cơ Bản</th>
                  <th className="p-3 border-r border-gray-600 text-center w-20">Ngày công</th>
                  <th className="p-3 border-r border-gray-600 text-center w-20">Giờ làm</th>
                  <th className="p-3 border-r border-gray-600 text-center w-20">Nghỉ phép</th>
                ...
                {payrollUsers.map((u, idx) => {
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-gray-750 group">
                      ...
                      <td className="p-1 border-r border-gray-600 text-center">
                        {renderInput(u.id, u.hr?.totalWorkHours || 0, 'totalWorkHours', 'number', 'text-center')}
                      </td>
                      <td className="p-1 border-r border-gray-600 text-center">
                        {renderInput(u.id, u.hr?.leaveDays || 0, 'leaveDays', 'number', 'text-center')}
                      </td>
