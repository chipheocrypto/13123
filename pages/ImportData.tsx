...
export const ImportData = () => {
  ...
  // --- DOWNLOAD LOGIC (SEQUENTIAL for Mobile Compatibility) ---
  ...
  const handleDownloadImage = (img: ImportImage) => {
    triggerDownload(img.url, `${img.description}.png`);
  };
  ...
  const handleDownloadSelected = async () => {
    ...
    if (confirm(`Bạn có chắc muốn tải xuống ${targetImages.length} ảnh đã chọn?\nLưu ý: Quá trình sẽ tải lần lượt từng ảnh để đảm bảo tương thích trên điện thoại.`)) {
        ...
        try {
            for (let i = 0; i < targetImages.length; i++) {
                ...
                try {
                    await triggerDownload(img.url, `${img.description}.png`);
                } catch (e) {
                    console.error("Skipping failed image:", img.id);
                }
                
                // Delay 2 seconds between downloads for mobile browsers to handle file saving
                if (i < targetImages.length - 1) {
                    await new Promise(resolve => setTimeout(resolve, 2000));
                }
            }
            
            setTimeout(() => alert("Hoàn tất tải xuống!"), 1000);
        } catch (error) {
                ...
                <div className="bg-gray-800 p-2 rounded-lg border border-gray-700 flex flex-wrap items-center gap-2">
                    ...
                    {selectedIds.length > 0 && (
                        <>
                            ...
                            >
                                {isBulkDownloading ? (
                                    <><Loader size={14} className="mr-2 animate-spin"/> Đang tải {downloadProgress}...</>
                                ) : (
                                    <><FolderDown size={14} className="mr-2"/> Tải {selectedIds.length} ảnh</>
                                )}
                            </button>
