...
export const Bills = () => {
  ...
  const executeWithSecurityCheck = (action: () => void) => {
    // If user has a second password set, require it
    if (user?.secondPassword) {
        setPendingAction(() => action);
        setSecurityPin('');
        setIsSecurityModalOpen(true);
    } else {
        // If no second password set, allow action (or you could block it, but standard behavior is allow)
        action();
    }
  };
  ...
  const handleConfirmPrint = () => {
    ...
    // Optional: Close modal after print command sent
    // setPrintingOrder(null); 
  };

  // --- EDIT BILL HANDLERS ---
  const handleOpenEdit = (order: Order, requestId?: string) => {
    // If it's Admin opening direct edit, require security check
    // If it's Staff opening APPROVED edit, no check needed (already approved)
    if (isAdminOrManager && !requestId) {
  ...
  const handleSaveEdit = () => {
    if (editingOrder) {
        ...
        // alert("Cập nhật hóa đơn thành công!"); // Removed alert for smoother flow
    }
  };
  ...
  return (
    <div className="p-6 h-full overflow-y-auto">
      ...
      {/* EDIT BILL MODAL (Actual Editing) */}
      {editingOrder && (
      ...
      {printingOrder && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[70] p-4 overflow-y-auto">
          <div className="flex flex-col max-h-full w-full max-w-sm">
            <div id="bill-preview-content" className="bg-white text-black p-4 w-full mx-auto shadow-2xl relative font-sans text-xs leading-tight">
              ...
              <div className="text-center mb-2">
                 ...
                 <h2 className="text-base font-bold uppercase mt-3 mb-1">HÓA ĐƠN TÍNH TIỀN {getRoomName(printingOrder.roomId).toUpperCase()}</h2>
                 {printingOrder.id && <p className="text-[10px]">Số: {printingOrder.id}</p>}
              </div>
