...
import { Product, RoomStatus, Role } from '../types';
import { 
...
export const RoomPos: React.FC<RoomPosProps> = ({ roomId, onBack, onChangeRoom }) => {
  ...
  const dispSubTotal = Math.round(subTotal);
    ...
    <div className="flex h-screen bg-gray-900 text-white overflow-hidden flex-col lg:flex-row">
            ...
            {filteredProducts.map(product => (
              <div 
                ...
                onClick={() => {
                  ...
                  // Optional: Switch to order tab on mobile after add? No, let user add multiple.
                }}
                className={`
      ...
      <div className={`w-full lg:w-2/5 flex flex-col bg-gray-800 h-[calc(100vh-60px)] lg:h-screen ${mobileTab === 'ORDER' ? 'flex' : 'hidden lg:flex'}`}>
          ...
          <div className="bg-gray-700 rounded-lg p-3">
            <div className="flex justify-between items-center mb-1">
               <div className="flex items-center space-x-1">
                  <span className="text-xs text-gray-400">Vào:</span>
                  <span className="text-sm font-mono font-bold text-white">{formatTime(order.startTime)}</span>
                  <div className="flex flex-col ml-1">
                     <button onClick={() => adjustOrderStartTime(roomId, 1)} className="text-gray-400 hover:text-white p-0.5 bg-gray-600 rounded-sm mb-0.5 h-3 w-4 flex items-center justify-center"><ChevronUp size={10}/></button>
                     <button onClick={() => adjustOrderStartTime(roomId, -1)} className="text-gray-400 hover:text-white p-0.5 bg-gray-600 rounded-sm h-3 w-4 flex items-center justify-center"><ChevronDown size={10}/></button>
                  </div>
               </div>
            ...
            <div className="flex items-center text-yellow-400 border-t border-gray-600 pt-2">
              ...
              <span className="text-xl font-mono font-bold tracking-wider">{formatDuration(durationMs)}</span>
            </div>
          </div>
        </div>
            ...
            <tbody>
              {order.items.map((item) => {
                ...
                if (item.isTimeBased && item.startTime) {
                   ...
                   const itemDurationMs = (item.endTime || currentTime) - item.startTime;
                   return (
                     <tr key={item.id} className="border-b border-gray-700/50 bg-indigo-900/10 text-sm">
                       <td className="py-3 pl-2">
                         <p className="font-medium text-indigo-300 truncate max-w-[120px]">{item.name}</p>
                         <div className="flex items-center space-x-2 mt-1">
                            <span className="text-[10px] text-gray-400 font-mono flex items-center">
                               {formatTime(item.startTime)}
                               <div className="flex flex-col ml-1">
                                 <button onClick={() => adjustOrderItemStartTime(roomId, item.id!, 1)} className="text-gray-500 hover:text-white p-0.5 bg-gray-700 rounded-sm mb-0.5 h-2 w-3 flex items-center justify-center"><ChevronUp size={8}/></button>
                                 <button onClick={() => adjustOrderItemStartTime(roomId, item.id!, -1)} className="text-gray-500 hover:text-white p-0.5 bg-gray-700 rounded-sm h-2 w-3 flex items-center justify-center"><ChevronDown size={8}/></button>
                               </div>
                            </span>
                            <span className="text-[10px] text-yellow-500 font-mono flex items-center">
                               <Clock size={10} className="mr-1"/> {formatDuration(itemDurationMs)}
                            </span>
                         </div>
                       </td>
                       <td className="py-3 text-center align-middle">
                         <div className="flex justify-center">
                           {isRunning ? (
                              <button onClick={() => stopServiceItem(roomId, item.id!)} className="text-yellow-500"><PauseCircle size={20} /></button>
                           ) : (
                              <button onClick={() => resumeServiceItem(roomId, item.id!)} className="text-green-500"><PlayCircle size={20} /></button>
                           )}
                         </div>
                       </td>
                       ...
                       <td className="py-3 text-center">
                         {canDeleteItems && (
                           <button onClick={() => removeItemFromOrder(roomId, item.id!)} className="text-red-500"><Trash2 size={14} /></button>
                         )}
                       </td>
                     </tr>
                   );
                } else {
                  const productForUpdate = { ...item, id: item.productId } as unknown as Product;
                  return (
                    <tr key={item.id || item.productId} className="border-b border-gray-700/50 text-sm">
                      ...
                      <td className="py-3 text-center">
                        <div className="flex items-center justify-center space-x-1">
                          <button onClick={() => addItemToOrder(roomId, productForUpdate, -1)} className="p-1 rounded bg-gray-600"><Minus size={10} /></button>
                          <span className="w-5 text-center text-xs">{item.quantity}</span>
                          <button onClick={() => addItemToOrder(roomId, productForUpdate, 1)} className="p-1 rounded bg-gray-600"><Plus size={10} /></button>
                        </div>
                      </td>
                      <td className="py-3 text-right pr-2 font-medium">{Math.round(itemTotal).toLocaleString()}</td>
                      <td className="py-3 text-center">
                        {canDeleteItems && (
                          <button onClick={() => removeItemFromOrder(roomId, item.id || item.productId)} className="text-red-500"><Trash2 size={14} /></button>
                        )}
                      </td>
                    </tr>
                  );
        ...
        <div className="bg-gray-800 border-t border-gray-700 p-4 shrink-0 pb-20 lg:pb-4">
          ...
          <button onClick={handleInitiatePayment} className="w-full flex items-center justify-center bg-pink-600 hover:bg-pink-500 text-white py-3 lg:py-4 rounded-lg font-bold shadow-lg text-lg uppercase">
             <CreditCard className="mr-2" size={24} /> THANH TOÁN
          </button>
        </div>
      </div>

      {/* Invoice Modal (Same as before, just kept responsive width) */}
      {showInvoice && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] p-4 overflow-y-auto">
          <div className="flex flex-col max-h-full w-full max-w-sm">
            <div id="print-section" className="bg-white text-black p-4 w-full mx-auto shadow-2xl relative font-sans text-xs leading-tight">
              {/* ... (Invoice Content Same as previous) ... */}
              <div className="text-center mb-2">
                 <h1 className="font-bold text-lg uppercase mb-1">{settings.storeName}</h1>
                 <p className="mb-1">{settings.storeAddress}</p>
                 <p className="mb-2 font-bold">ĐT: {settings.storeTaxCode}</p>
                 <h2 className="text-base font-bold uppercase mt-3 mb-1">HÓA ĐƠN TÍNH TIỀN {room.name.toUpperCase()}</h2>
              </div>
              <div className="mb-3 border-b border-black pb-1">
                 <div className="flex justify-between"><span>Giờ vào:</span><span>{formatDateTime(order.startTime)}</span></div>
                 <div className="flex justify-between"><span>Giờ ra:</span><span>{formatDateTime(currentTime)}</span></div>
              ...
              <div className="border-t-2 border-dashed border-black py-2 mb-2">
                 ...
                 <div className="flex mb-1">
                    ...
                    <span className="w-[25%] text-center text-[10px]">{formatTime(order.startTime)} - {formatTime(currentTime)}</span>
            ...
            <div className="no-print mt-4 flex justify-center space-x-2 pb-8">
              <button onClick={handlePrint} className="flex-1 bg-gray-800 text-white py-3 rounded text-sm">In Bill</button>
              <button onClick={handleCompleteCheckout} className="flex-1 bg-pink-600 text-white py-3 rounded font-bold text-sm">Hoàn Tất</button>
              <button onClick={() => setShowInvoice(false)} className="bg-gray-600 text-white px-4 py-3 rounded text-sm">Đóng</button>
            </div>
          </div>
        </div>
      )}
