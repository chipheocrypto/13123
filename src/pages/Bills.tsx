...
import { Calendar, FileText, Clock, AlertTriangle, Check, X, Eye, Edit, Trash2, Plus, Minus, History, Save, Printer, Lock, ChevronUp, ChevronDown, ShieldCheck } from 'lucide-react';
...
export const Bills = () => {
  ...
  const isHardLocked = (endTime?: number) => {
    ...
    const now = Date.now();
    const diffMinutes = (now - endTime) / 60000;
    return diffMinutes > (settings.hardBillLockMinutes || 1440); 
  };
    ...
    <div className="p-6 h-full overflow-y-auto">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-3xl font-bold text-white flex items-center">
          <FileText className="mr-3 text-pink-500" /> Quản Lý Hóa Đơn
        </h2>
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-700 text-sm">
              {dailyOrders.map(order => {
                const request = billRequests.find(r => r.orderId === order.id && r.status !== 'COMPLETED');
