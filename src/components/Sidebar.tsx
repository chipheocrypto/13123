...
import { 
  ...
  Receipt,
  X,
  FileText, // Changed from ReceiptText to FileText to fix import error
  Database
} from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ currentPage, setPage, isOpen, onClose }) => {
  ...
  const menuItems = [
    ...
    { id: 'bills', label: 'Hóa đơn & Duyệt', icon: FileText, badge: pendingRequests > 0 && user?.role !== Role.STAFF ? pendingRequests : 0 },
    { id: 'inventory', label: 'Kho hàng', icon: ShoppingBag },
    { id: 'import-data', label: 'Data Nhập Hàng', icon: Database },
  ...
  const checkPermission = (itemId: string) => {
    ...
    switch (itemId) {
      ...
      case 'bills': return perms.includes('bills');
      
      case 'inventory': return perms.includes('inventory');
      case 'import-data': return perms.includes('import_data');
      case 'daily-expenses': return perms.includes('daily_expenses');
