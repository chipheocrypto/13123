...
export const AppProvider = ({ children }: { children?: ReactNode }) => {
  ...
  const login = (username: string, password: string, remember: boolean) => {
    ...
    if (foundUser) {
      ...
      // 1. Nếu đang ở trang Staff (staff.domain.com)
      if (hostname.startsWith('staff.')) {
          if (foundUser.role === Role.ADMIN || foundUser.role === Role.SUPER_ADMIN) {
              setLoginError("Tài khoản Admin vui lòng đăng nhập tại trang quản trị (admin.domain...)");
              return false;
          }
      }
      
      // 2. Nếu đang ở trang Admin (admin.domain.com)
      if (hostname.startsWith('admin.')) {
          if (foundUser.role === Role.STAFF) {
              setLoginError("Tài khoản Nhân viên vui lòng đăng nhập tại cổng nhân viên (staff.domain...)");
              return false;
          }
      }
      // ---------------------------
